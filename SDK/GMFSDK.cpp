...
namespace GMFSDK
{
	...
	uint64_t GMFMesh::AddVertex(const float x, const float y, const float z, const float nx, const float ny, const float nz, const float u0, const float v0, const float u1, const float v1, const unsigned char r, const unsigned char g, const unsigned char b, const unsigned char a, const float displacement)
	{
		...
		vertices[sz].normal[0] = nx * 127.0f;
		vertices[sz].normal[1] = ny * 127.0f;
		vertices[sz].normal[2] = nz * 127.0f;
		vertices[sz].texcoords[0] = floatToHalf(u0);
		vertices[sz].texcoords[1] = floatToHalf(v0);
	...
	{
		if (polyverts != 3 || polyverts != 4) return false;
		...
		for (i = 0; i < polycount; i++)
		{
			...
			if ((vertexupdated[a] == false) || (vertexupdated[b] == false) || (vertexupdated[c] == false))
			{
				...
				texcoords.x = halfToFloat(vertices[a].texcoords[0]);
				texcoords.y = halfToFloat(vertices[a].texcoords[1]);
				v1x = position.x; v1y = position.y; v1z = position.z; v1u = texcoords.x; v1v = texcoords.y;

				position = vertices[b].position;
				texcoords.x = halfToFloat(vertices[b].texcoords[0]);
				texcoords.y = halfToFloat(vertices[b].texcoords[1]);
				v2x = position.x; v2y = position.y; v2z = position.z; v2u = texcoords.x; v2v = texcoords.y;

				position = vertices[c].position;
				texcoords.x = halfToFloat(vertices[c].texcoords[0]);
				texcoords.y = halfToFloat(vertices[c].texcoords[1]);
				...
				if (vertexupdated[a] == false)
				{
					...
					vertices[a].tangent[0] = floatToHalf(s.x);
					vertices[a].tangent[1] = floatToHalf(s.y);
					vertices[a].tangent[2] = floatToHalf(s.z);
					vertices[a].tangent[3] = floatToHalf(sgna);
				}
				if (vertexupdated[b] == false)
				{
					vertexupdated[b] = true;
					vertices[b].tangent[0] = floatToHalf(s.x);
					vertices[b].tangent[1] = floatToHalf(s.y);
					vertices[b].tangent[2] = floatToHalf(s.z);
					vertices[b].tangent[3] = floatToHalf(sgnb);
				}
				if (vertexupdated[c] == false)
				{
					vertexupdated[c] = true;
					vertices[c].tangent[0] = floatToHalf(s.x);
					vertices[c].tangent[1] = floatToHalf(s.y);
					vertices[c].tangent[2] = floatToHalf(s.z);
