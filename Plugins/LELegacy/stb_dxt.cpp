...
// Alpha block compression (this is easy for a change)
static void stb__CompressAlphaBlock_S(unsigned char* dest, unsigned char* src, int stride)
{
    int i, dist, bias, dist4, dist2, bits, mask;
    ...
    // encode them
    ((signed char*)dest)[0] = mx;
    ((signed char*)dest)[1] = mn;
    dest += 2;
...
{
    stb__CompressAlphaBlock(dest, (unsigned char*)src, 4);
    stb__CompressAlphaBlock(dest + 8, (unsigned char*)src + 1, 4);
}

//-------------------------------------------------------------------
// New stuff below this line - Josh
//-------------------------------------------------------------------
...
static void extractBlock1(const unsigned char* src, int x, int y,
    int w, int h, unsigned char* block)
{
    ...
    if ((w - x >= 1) && (h - y >= 1))
    {
        ...
        for (i = 0; i < 1; ++i)
        {
            *(unsigned int*)block = *(unsigned int*)src; block += 1; src += 1;
            *(unsigned int*)block = *(unsigned int*)src; block += 1; src += 1;
            *(unsigned int*)block = *(unsigned int*)src; block += 1; src += 1;
            *(unsigned int*)block = *(unsigned int*)src; block += 1;
            src += (w * 1) - 3;
        }
    }
}

static void extractBlock2(const unsigned char* src, int x, int y,
    int w, int h, unsigned char* block)
{
    int i, j;
    if ((w - x >= 2) && (h - y >= 2))
    {
        ...
        for (i = 0; i < 2; ++i)
        {
            *(unsigned int*)block = *(unsigned int*)src; block += 2; src += 2;
            *(unsigned int*)block = *(unsigned int*)src; block += 2; src += 2;
            *(unsigned int*)block = *(unsigned int*)src; block += 2; src += 2;
            *(unsigned int*)block = *(unsigned int*)src; block += 2;
            src += (w * 2) - 6;
        }
    }
}
...
/// <summary>
/// Expects RGBA8 format
/// </summary>
/// <param name="dst"></param>
/// <param name="src"></param>
/// <param name="w"></param>
/// <param name="h"></param>
void rygCompressBC1(unsigned char* dst, unsigned char* src, int w, int h)
{
    //const int isDxt5 = 0; // isDxt5 variable means "is 16 bytes" (and not 8)
...
/// <param name="w"></param>
/// <param name="h"></param>
void rygCompressBC3(unsigned char* dst, unsigned char* src, int w, int h)
{
    //const int isDxt5 = 1;
...
/// <param name="w"></param>
/// <param name="h"></param>
void rygCompressBC4(unsigned char* dst, unsigned char* src, int w, int h)
{
    //const int isDxt5 = 0;
...
/// <param name="w"></param>
/// <param name="h"></param>
void rygCompressBC5(unsigned char* dst, unsigned char* src, int w, int h)
{
    //const int isDxt5 = 1;
    ...
    /*for (y = 0; y < h; y += 2)
    {
        for (x = 0; x < w; x += 2)
        {
            extractBlock2(src, x, y, w, h, block);
            stb_compress_bc5_block(dst, block);
            dst += 16;
        }
    }*/
